...
Solution:           Store each integer and its corresponding index in nums into a vector of <int, int> pairs, where the integers are 
                    stored as keys and the indices are stored as values. Then, in order to use the two pointer technique, we must sort 
                    the vector by value in ascending order, which will give O(log(N)) time complexity. Then we use the two pointer
                    technique on the vector by iteratively adding the smaller values in the vector with the larger values and comparing 
                    the sums to the target value. If the sum is less than the target, add the next smallest value to the larger value;
                    if the sum is less than the target, add the next largest value to the smaller value; and if the sum is equal to the
                    target, return the indices of the integers used to produce that sum. 

Time complexity:    O(N * log(N)); N = the number of integers in nums

Space complexity:   O(N); N = the number of integers in the vector of <int, int> pairs
...
vector<int> twoSum(vector<int>& nums, int target)
{
    ...
    // Sort numPairs by value in ascending order.
